...
export function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      } catch { setData({ status: 'error', version: getVersion() }); }
      try { const info = await getProviderInfo(); setProviders(info); } catch {}
      try { const models = await getOllamaModels(); setOllamaModels(models); } catch {}
      try {
      ...
      } catch { setCacheStats('unavailable'); }
      try {
        ...
        } else { setConfigStatus('not found'); }
      } catch { setConfigStatus('error reading'); }
      try {
  ...
  useInput((_input, key) => {
    if (key.escape || _input === 'q') { navigate('/'); }
  });
    ...
    <Box flexDirection="column" width="100%" padding={2}>
      ...
      <Box flexDirection="row" gap={2} flexGrow={1}>
        <Box flexDirection="column" width="50%">
          <Box borderStyle="single" borderColor={colors.dimSeparator} padding={1} flexDirection="column" marginBottom={1}>
          ...
          <Box borderStyle="single" borderColor={colors.dimSeparator} padding={1} flexDirection="column">
        ...
        <Box flexDirection="column" width="50%">
          <Box borderStyle="single" borderColor={colors.dimSeparator} padding={1} flexDirection="column" marginBottom={1}>
            ...
            <Text dimColor>{sessionCount > 0 ? `${sessionCount} saved session(s)` : 'No recent activity'}</Text>
          </Box>

          <Box borderStyle="single" borderColor={colors.dimSeparator} padding={1} flexDirection="column" marginBottom={1}>
            <Text bold color={colors.success}>{'Ollama Models'}</Text>
