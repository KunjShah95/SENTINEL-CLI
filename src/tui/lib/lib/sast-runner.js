...
export function severityFromString(s) {
  ...
  if (lower === 'critical')
    return 'critical';
  if (lower === 'high' || lower === 'error' || lower === '2')
    return 'high';
  if (lower === 'medium' || lower === 'moderate' || lower === 'warning' || lower === '1')
    return 'medium';
  if (lower === 'low' || lower === 'info' || lower === '0')
    return 'low';
  return 'info';
}
// ─── 1. ESLint ────────────────────────────────────────────────────────────────
function runEslint(cwd) {
  ...
  const hasEslintRc = existsSync(join(cwd, '.eslintrc')) ||
...
function getTrackedFiles(cwd) {
  ...
  return stdout
    ...
    .filter(Boolean)
    .filter(f => {
      if (EXCLUDE_DIRS.test(f))
        return false;
      const ext = f.slice(f.lastIndexOf('.'));
      return SCAN_EXTENSIONS.has(ext);
    });
}
function runSecretScan(cwd) {
...
function hasMatchingFiles(cwd, exts) {
  const { stdout } = tryExec('git ls-files 2>/dev/null || echo .', { cwd });
  if (!stdout)
    return false;
  return stdout.split('\n').some(f => exts.some(ext => f.trim().endsWith(ext)));
}
function runBandit(cwd) {
  ...
  try {
    ...
    for (const r of results) {
      ...
      if (rawConf !== 'HIGH' && rawConf !== 'MEDIUM')
        continue;
      if (rawSev === 'LOW')
        continue;
      findings.push({
...
function runHadolint(cwd) {
  ...
  for (const df of dockerFiles) {
    ...
    try {
      ...
      if (jsonStart === -1 || jsonEnd <= jsonStart)
        continue;
      const results = JSON.parse(stdout.slice(jsonStart, jsonEnd));
      for (const r of results) {
        const level = r.level?.toLowerCase() || 'info';
        if (level === 'info' || level === 'style')
          continue;
        findings.push({
          tool: 'hadolint',
...
function runClippy(cwd) {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (!parsed.message || parsed.reason === 'compiler-artifact')
        continue;
      ...
      if (!clippyCode.startsWith('clippy::'))
        continue;
  ...
  try {
    ...
    if (result.vulnerabilities) {
      for (const [, vuln] of Object.entries(result.vulnerabilities)) {
        ...
        if (sev === 'low' || sev === 'info')
          continue; // skip low/info to reduce noise
...
export function filterDismissed(findings) {
...
export async function runSast(options = {}) {
...
export function formatSastForPrompt(result) {
