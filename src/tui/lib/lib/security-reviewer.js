...
export function buildReviewPrompt(diff, options = {}) {
...
export function parseSeverityFromEmoji(text) {
  if (text.includes('🔴') || text.toLowerCase().includes('critical'))
    return 'critical';
  if (text.includes('🟠') || text.toLowerCase().includes('high'))
    return 'high';
  if (text.includes('🟡') || text.toLowerCase().includes('medium'))
    return 'medium';
  if (text.includes('🟢') || text.toLowerCase().includes('low') || text.toLowerCase().includes('suggestion'))
    return 'low';
  return 'info';
}
// Parse a review response into structured issues
export function parseReviewResponse(text, files = []) {
  const issues = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line.includes('🔴') || (line.startsWith('#') && line.toLowerCase().includes('critical')))
      curSev = 'critical';
    else if (line.includes('🟠') || (line.startsWith('#') && line.toLowerCase().includes('high')))
      curSev = 'high';
    else if (line.includes('🟡') || (line.startsWith('#') && line.toLowerCase().includes('medium')))
      curSev = 'medium';
    else if (line.includes('🟢') || (line.startsWith('#') && (line.toLowerCase().includes('low') || line.toLowerCase().includes('suggestion'))))
      curSev = 'low';
...
export function getSeverityColorKey(sev) {
  switch (sev) {
...
// Get score color
export function getScoreColor(text) {
