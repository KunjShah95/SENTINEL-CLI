...
import { runReviewLoop, startWatchLoop, runPipelineLoop, runCILoop } from '../lib/loop-engine.js';
export function useLoopEngine() {
  ...
  const submitAndWait = useCallback(async (prompt, loopMode = 'REVIEW') => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (after.some(m => m.role === 'assistant'))
        break;
    }
    ...
    // ── 1. Review Loop ──────────────────────────────────────────────────────────
  const startReviewLoop = useCallback((opts = {}) => {
    ...
    // ── 2. Watch Loop ───────────────────────────────────────────────────────────
  const startWatchLoop_ = useCallback((opts = {}) => {
    ...
    // ── 3. Pipeline Loop ────────────────────────────────────────────────────────
  const startPipelineLoop = useCallback((opts = {}) => {
    abortRef.current = { aborted: false };
    setIsRunning(true);
    ...
    // ── 4. CI Loop ──────────────────────────────────────────────────────────────
  const startCILoop = useCallback((opts = {}) => {
    abortRef.current = { aborted: false };
    setIsRunning(true);
    setLoopType('ci');
    setEvents([]);
