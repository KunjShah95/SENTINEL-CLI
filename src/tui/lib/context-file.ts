...
export type ContextFile = {
  path: string;
  content: string;
  source: 'SENTINEL.md' | 'CLAUDE.md' | '.sentinel/context.md' | 'custom';
};
...
const CONTEXT_FILE_NAMES: Array<ContextFile['source']> = [
  'SENTINEL.md',
  'CLAUDE.md',
  '.sentinel/context.md',
];
 ...
 */
export function buildContextInjection(files: ContextFile[]): string {
 ...
 */
export function injectContextIntoPrompt(prompt: string, cwd?: string): string {
 ...
 */
export function createDefaultContextFile(cwd?: string): void {
  const dir = resolve(cwd ?? process.cwd());

  // Don't create if any context file already exists
  for (const source of CONTEXT_FILE_NAMES) {
    if (existsSync(join(dir, source))) return;
  }
