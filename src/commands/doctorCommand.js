...
export class DoctorCommand {
    ...
    async checkApiKeys() {
        ...
        return {
            ...
            details: 'Run "sentinel auth" to configure API keys'
        };
    }
    ...
    async checkDependencies() {
        try {
            ...
            const { stdout } = await execAsync('npm ls --depth=0 2>/dev/null | head -20', { cwd: this.projectPath });
    ...
    displayResults() {
        ...
        for (const check of this.checks) {
            ...
            console.log(
                chalk.white(`  ${check.name.padEnd(30)}`) + 
                statusIcon + ' ' + chalk.gray(check.message)
            );
        }
    ...
    getSummary() {
        ...
        if (failCount === 0 && warnCount === 0) {
            ...
            console.log(chalk.yellow(`  ⚠️  ${warnCount} warning(s) - Sentinel may have limited functionality.\n`));
        } else {
