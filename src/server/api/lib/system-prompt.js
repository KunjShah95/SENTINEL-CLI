...
export function buildSystemPrompt({ mode }) {
  ...
  if (mode === Mode.PLAN) {
    ...
    modeDescription = [
      ...
      'Use read tools to verify context when needed.',
      '',
      ...
      '  Description and security impact',
      ...
      '',
      '### 🟢 Low / Suggestions',
      '- **[file:line]** Issue title  Description',
      '',
      '## Security Checklist',
      '- [x/✗] SQL injection',
  ...
  if (mode === Mode.BUILD) {
    ...
    rulesArray.push(
      'Use diffFile to preview changes before applying them when the user asks for a preview.'
    );
    rulesArray.push(
      'AUTOINSTALL: If a bash command fails due to a missing package or command, immediately use bash to install the dependency and try again.'
    );
    rulesArray.push(
      'CHECKPOINTS: All file edits are automatically checkpointed. The user can undo with /undo or you can use the undoLastChange tool.'
    );
  } else {
    rulesArray.push(
