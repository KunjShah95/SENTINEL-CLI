...
function filterFiles(files, token, limit) {
    if (!token)
...
export function InputBar({ onSubmit, onCommand, onSlashCommand, disabled = false, placeholder = "Ask Sentinel to do anything...", mode = "BUILD", onModeToggle, onCommandPalette, }) {
        ...
        (async () => {
            try {
                ...
                if (initial?.truncated || files.length >= LARGE_PROJECT_THRESHOLD) {
                    ...
                    if (cancelled)
                        return;
                    const lazyFiles = Array.isArray(lazyResult?.files) ? lazyResult.files : [];
    ...
    const handleInput = useCallback((next) => {
    ...
    return (_jsxs("box", { flexDirection: "column", width: "100%", children: [showSuggestions ? (_jsx("box", { flexDirection: "column", backgroundColor: colors.dialogSurface, paddingX: 1, width: "100%", children: suggestions.map((filePath, i) => {
                    const isSelected = i === selectedIndex;
                    return (_jsxs("text", { fg: isSelected ? colors.primary : colors.info, attributes: isSelected ? TextAttributes.BOLD : TextAttributes.DIM, children: [isSelected ? "> " : "  ", filePath] }, filePath));
                }) })) : null, _jsxs("box", { flexDirection: "row", width: "100%", alignItems: "center", children: [_jsxs("text", { fg: activeColor, children: ["\u276F", " "] }), _jsx("input", { ref: inputRef, placeholder: disabled ? "Processing..." : placeholder, focused: !disabled, flexGrow: 1, onInput: handleInput, onSubmit: handleSubmit })] })] }));
}
