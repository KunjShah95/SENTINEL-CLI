 ...
 * Now includes real-time CVE scanning via npm audit.
 */
export class DependencyAnalyzer extends BaseAnalyzer {
  constructor(config) {
    ...
    this.useRealTimeCve = config?.get?.('analysis.realTimeCve') !== false;
  }
  ...
  async analyze(files, context) {
    ...
    // Run real-time CVE check if enabled and package.json exists
    if (this.useRealTimeCve) {
  ...
  /**
   * Run npm audit for real-time CVE detection
   */
  async runNpmAudit() {
    try {
      ...
      if (error.stdout) {
        try {
          ...
          // Could not parse, fallback to static checks
          console.warn('[DependencyAnalyzer] npm audit parsing failed, using static checks');
        }
      } else {
        // npm not available or network error - that's okay, we have static checks
        console.warn('[DependencyAnalyzer] npm audit unavailable, using static checks only');
      }
    }
  }

  /**
   * Parse npm audit JSON output and add issues
   */
      ...
      if (vuln.via && Array.isArray(vuln.via)) {
        for (const via of vuln.via) {
          if (typeof via === 'object') {
            this.addIssue({
              severity: this.mapNpmSeverity(via.severity || vuln.severity),
              type: 'dependency',
              title: `CVE: ${via.title || pkgName}`,
              message: via.title || `Vulnerability in ${pkgName}`,
              file: 'package.json',
              line: 1,
              suggestion: vuln.fixAvailable
                ? `Fix available: ${typeof vuln.fixAvailable === 'object' ? vuln.fixAvailable.name : 'run npm audit fix'}`
                : 'Update to a patched version',
              tags: ['dependency', 'security', 'cve', 'npm-audit'],
              analyzer: this.name,
              metadata: {
                cwe: via.cwe,
                url: via.url,
        ...
        this.addIssue({
          ...
          message: vuln.overview || vuln.title || `Security issue in ${pkgName}`,
          file: 'package.json',
          line: 1,
          suggestion: vuln.recommendation || 'Update to a patched version',
          tags: ['dependency', 'security', 'cve', 'npm-audit'],
          analyzer: this.name,
        });
      }
  ...
  /**
   * Map npm severity levels to Sentinel severity levels
   */
