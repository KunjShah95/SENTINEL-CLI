...
function registerCommands(context, services) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.analyze', async () => {
        ...
        try {
            vscode.window.withProgress({
                ...
                vscode.window.showInformationMessage(`Analysis complete: ${summary.total} issues found (${summary.critical} critical, ${summary.high} high)`);
            });
        }
        catch (error) {
    ...
    // Analyze folder
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.analyzeFolder', async (uri) => {
        ...
        try {
            vscode.window.withProgress({
                ...
                diagnostics.updateDiagnostics(result.issues);
                sidebarProvider.updateIssues(result.issues);
                vscode.window.showInformationMessage(`Folder analysis complete: ${result.summary.total} issues found`);
            });
        }
        catch (error) {
    ...
    // Security audit
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.securityAudit', async () => {
        try {
            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                ...
                diagnostics.updateDiagnostics(result.issues);
                sidebarProvider.updateIssues(result.issues);
                vscode.window.showInformationMessage(`Security audit complete: ${result.summary.total} vulnerabilities found`);
            });
        }
        catch (error) {
    ...
    // Full scan
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.fullScan', async () => {
        try {
            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                ...
                diagnostics.updateDiagnostics(result.issues);
                sidebarProvider.updateIssues(result.issues);
                vscode.window.showInformationMessage(`Full scan complete: ${result.summary.total} issues found`);
            });
        }
        catch (error) {
    ...
    // Auto fix
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.autoFix', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    // Explain code
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.explainCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    // Refactor code
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.refactorCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No code selected');
            return;
    ...
    // Generate tests
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.generateTests', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No file open');
            return;
    ...
    // Document code
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.documentCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No file open');
            return;
    ...
    // Scan secrets
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.scanSecrets', async () => {
        try {
            const result = await sentinelService.scanSecrets();
            diagnostics.updateDiagnostics(result.issues);
            sidebarProvider.updateIssues(result.issues);
            vscode.window.showInformationMessage(`Secret scan complete: ${result.summary.total} potential secrets found`);
        }
        catch (error) {
    ...
    // Pre-commit check
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.preCommit', async () => {
        try {
            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                ...
                if (result.blocked) {
                    vscode.window.showErrorMessage(`Pre-commit check failed: ${result.issues.length} blocking issues found`);
                }
                else {
    ...
    // Refresh sidebar
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.refreshSidebar', async () => {
        try {
            const result = await sentinelService.fullScan();
            diagnostics.updateDiagnostics(result.issues);
            sidebarProvider.updateIssues(result.issues);
    ...
    // Apply fix (internal)
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.applyFix', async (issue) => {
        if (!issue.fix) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('sentinel.ignoreIssue', async (issue) => {
        const config = vscode.workspace.getConfiguration('sentinel');
        const ignored = config.get('ignoredIssues', []);
